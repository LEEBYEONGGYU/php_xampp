...
${'menu_item_srl31_argument'} = new Argument('menu_item_srl', $args->{'menu_item_srl'});
${'menu_item_srl31_argument'}->checkFilter('number');
${'menu_item_srl31_argument'}->checkNotNull();
if(!${'menu_item_srl31_argument'}->isValid()) return ${'menu_item_srl31_argument'}->getErrorMessage();
if(${'menu_item_srl31_argument'} !== null) ${'menu_item_srl31_argument'}->setColumnType('number');

${'parent_srl32_argument'} = new Argument('parent_srl', $args->{'parent_srl'});
${'parent_srl32_argument'}->checkFilter('number');
${'parent_srl32_argument'}->ensureDefaultValue('0');
if(!${'parent_srl32_argument'}->isValid()) return ${'parent_srl32_argument'}->getErrorMessage();
if(${'parent_srl32_argument'} !== null) ${'parent_srl32_argument'}->setColumnType('number');

${'menu_srl33_argument'} = new Argument('menu_srl', $args->{'menu_srl'});
${'menu_srl33_argument'}->checkFilter('number');
${'menu_srl33_argument'}->checkNotNull();
if(!${'menu_srl33_argument'}->isValid()) return ${'menu_srl33_argument'}->getErrorMessage();
if(${'menu_srl33_argument'} !== null) ${'menu_srl33_argument'}->setColumnType('number');
...
if(${'name34_argument'} !== null) ${'name34_argument'}->setColumnType('text');
if(isset($args->desc)) {
${'desc35_argument'} = new Argument('desc', $args->{'desc'});
if(!${'desc35_argument'}->isValid()) return ${'desc35_argument'}->getErrorMessage();
} else
${'desc35_argument'} = NULL;if(${'desc35_argument'} !== null) ${'desc35_argument'}->setColumnType('varchar');
if(isset($args->url)) {
...
new InsertExpression('`menu_item_srl`', ${'menu_item_srl31_argument'})
,new InsertExpression('`parent_srl`', ${'parent_srl32_argument'})
,new InsertExpression('`menu_srl`', ${'menu_srl33_argument'})
,new InsertExpression('`name`', ${'name34_argument'})
,new InsertExpression('`desc`', ${'desc35_argument'})
...
new Table('`decoel_menu_item`', '`menu_item`')
));
