...
class multimedia_link extends EditorHandler
{
	...
	function transHTML($xml_obj)
	{
		...
		if(Context::getResponseMethod() != "XMLRPC" && substr ($src,-4) == ".mp3" ) return sprintf("<audio id=\"player2\" src=\"%s\" type=\"audio/mp3\" controls=\"controls\" %s></audio><br />", $src, $player_auto);
		else if(Context::getResponseMethod() != "XMLRPC" && substr ($src,-4) == ".mp4" ) return sprintf("<video class=\"mejs-wmp\" src=\"%s\" width=\"%s\" height=\"%s\" type=\"video/mp4\" id=\"player1\" controls=\"controls\" preload=\"none\"></video>",$src, $width, $height);
